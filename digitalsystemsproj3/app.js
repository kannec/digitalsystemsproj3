(function () {
  ...
  function tier2Suggestions(parentLabel) {
    ...
    return pickThree(pool.concat(hintTerms(taggingState.imageHints)), hints, []);
  }
  ...
  function taggingSyncFromImage(editingPin) {
    ...
    if (editingPin) {
      ...
      if (taggingState.tier1Pick) {
        ...
        if (t1 && !taggingState.tier2.includes(t1)) {
  ...
  function refreshTier2() {
    ...
    const avoid = new Set(taggingState.tier2);
  ...
  function collectTagsFromTagging() {
    ...
    taggingState.tier1.forEach((t) => {
      if (!tags.includes(t)) tags.push(t);
    });
    taggingState.tier2.forEach((t) => {
      if (!tags.includes(t)) tags.push(t);
    });
    return tags;
  }
  ...
  el("btn-add").addEventListener("click", () => fileInput.click());
